...
function ViewTrainings() {
    ...
    useEffect(()=>{
        axios.get("/readTraining").then((response)=>{
            ...
            const trainingsList = response.data.data;
            
            for (let index = 0; index < trainingsList.length; index++) {
                if (trainingsList[index]._id === trainingID) {
                    setTrainingData(trainingsList[index]);
    ...
    return (
        <div className={style.parent}>
            ...
            <div className={style.subparent}>
                <p className={style.redtxt}>{trainingData?.TrainingName}</p>
                        ...
                        <div className={style.sec2} >
                            <div>
                                ...
                                <div>
                                    ...
                                    <p className={style.card1para2}>{trainingData?.EvaluationCriteria}</p>
                                </div>
                            </div>
                            <div>
                                <img src={copy} alt="" />
                                <div>
                                    ...
                                    <p className={style.card1para2}>{trainingData?.Description}</p>
                                </div>
                            </div>
                            <div>
                                <img src={office} alt="" />
