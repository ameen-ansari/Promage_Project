...
router.post('/addMachinery', async (req, res) => {
  try {
    ...
    const { machineName, machinaryLocation, maintainancefrequencey } = req.body;
    ...
    res.status(201).send({
      status: true,
      message: 'The Machinery is added!',
      data: savedMachinery,
    });
  } catch (error) {
    // Log the error
...
router.get('/readAllMachinery', async (req, res) => {
  try {
    const equipment = await Machinery.find();
    const totalCollections =  await Machinery.countDocuments();

    // Log successful retrieval of data
    console.log('Successfully fetched machinery data.');

    res.status(201).send({ status: true, message: "The following are Machinery!", totaldocuments: totalCollections, data: equipment });
  } catch (error) {
