...
router.post('/addEquipment', async (req, res) => {
  ...
  const { equipmentName, equipmentLocation, equipmentMaintenence, calibrationType, equipmentFrequency } = req.body;

  // Check if all required fields are provided
  if (!equipmentName || !equipmentLocation || !equipmentMaintenence || !calibrationType || !equipmentFrequency) {
...
router.get('/readAllEquipment', async (req, res) => {
  try {
    ...
    // Get the total number of equipment documents in the collection
    const totalCollections = await Equipment.countDocuments();

    // Log the number of total documents in the collection
    console.log('Total Equipment documents:', totalCollections);

    // Send the response with the equipment data
    res.status(200).json({ status: true, message: 'Successfully retrieved all equipment!', totalDocuments: totalCollections, data: equipment });
  } catch (error) {
